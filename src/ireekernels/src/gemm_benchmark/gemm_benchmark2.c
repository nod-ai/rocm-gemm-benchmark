...
iree_status_t ireeGemmRuntimeCallback(void* user_data, iree_loop_t loop,
  ...
  runtime_state_t* s = (runtime_state_t*)user_data;
...
void ireeGemmRuntimeWaitForCompletion(runtime_state_t* s) {
  pthread_mutex_lock(s->lock);
...
int ireeGemmRuntimeInitialize(const char* device_uri,
  ...
  state->device = device;
  state->storage = (runtime_storage_t*)malloc(sizeof(runtime_storage_t));
  *state_ref = state;
  ...
  state->cond = (pthread_cond_t*)malloc(sizeof(pthread_cond_t));
...
int ireeGemmRuntimeCreateInputs(runtime_state_t* s,
  ...
  // create subspans
  status = iree_hal_buffer_subspan(storage->bucket_A, storage->offset_A,
                                   storage->size_A, &storage->A);
  CHECK_IREE_STATUS(status, "Failed to create buffer subspan A");
  status = iree_hal_buffer_subspan(storage->bucket_B, storage->offset_B,
                                   storage->size_B, &storage->B);
  ...
  status = iree_hal_buffer_view_create(storage->A, 2, shape_A, storage->dtype,
                                       IREE_HAL_ENCODING_TYPE_DENSE_ROW_MAJOR,
                                       iree_allocator_system(),
                                       &input_buffer_view_A);
  CHECK_IREE_STATUS(status, "Failed to create buffer view A");
  status = iree_hal_buffer_view_create(storage->B, 2, shape_B, storage->dtype,
                                       IREE_HAL_ENCODING_TYPE_DENSE_ROW_MAJOR,
                                       iree_allocator_system(),
                                       &input_buffer_view_B);
...
int ireeGemmRuntimeDispatchIOBuffers(runtime_state_t* s, int M, int K, int N,
  ...
  storage->offset_A = 0;
  storage->offset_B = 0;
  storage->size_A = size_A;
  storage->size_B = size_B;
  storage->dtype = parsed_dtype;
  storage->capacity = 1e8;
  ...
  // Allocate space in device memory for buffers
  iree_device_size_t allocation_size_A = 0;
  status = iree_hal_buffer_compute_view_size(
      2, shape_A, parsed_dtype, IREE_HAL_ENCODING_TYPE_DENSE_ROW_MAJOR,
      &allocation_size_A);
  iree_device_size_t allocation_size_B = 0;
  status = iree_hal_buffer_compute_view_size(
      2, shape_B, parsed_dtype, IREE_HAL_ENCODING_TYPE_DENSE_ROW_MAJOR,
      &allocation_size_B);
  status = iree_hal_allocator_allocate_buffer(
      allocator, device_buffer_params, allocation_size_A, &storage->bucket_A);
  CHECK_IREE_STATUS(status, "Failed to allocate buffer A");
  status = iree_hal_allocator_allocate_buffer(
      allocator, device_buffer_params, allocation_size_B, &storage->bucket_B);
  CHECK_IREE_STATUS(status, "Failed to allocate buffer B");

  // Write data to buffers
  iree_const_byte_span_t byte_span_A = iree_make_const_byte_span(A, size_A);
  iree_const_byte_span_t byte_span_B = iree_make_const_byte_span(B, size_B);
  iree_hal_device_transfer_h2d(
      s->device, byte_span_A.data, storage->bucket_A, 0,
      iree_min(byte_span_A.data_length, allocation_size_A),
      IREE_HAL_TRANSFER_BUFFER_FLAG_DEFAULT, iree_infinite_timeout());
  iree_hal_device_transfer_h2d(
      s->device, byte_span_B.data, storage->bucket_B, 0,
      iree_min(byte_span_B.data_length, allocation_size_B),
      IREE_HAL_TRANSFER_BUFFER_FLAG_DEFAULT, iree_infinite_timeout());

  // Create inputs and outputs
  int input_err = ireeGemmRuntimeCreateInputs(s, shape_A, shape_B, &s->inputs);
...
void ireeGemmRuntimeExecute(runtime_state_t* s, int num_iterations) {
  ...
  s->completed_iterations = 0;
  s->start_time = clock();

  iree_vm_async_invoke_state_t states[num_iterations];
  for (int i = 0; i < num_iterations; ++i) {
    iree_vm_async_invoke(loop, &states[i], s->context, s->function,
                         IREE_VM_INVOCATION_FLAG_NONE, NULL, s->inputs,
                         s->outputs, iree_allocator_system(),
                         ireeGemmRuntimeCallback, s);
  }

  ireeGemmRuntimeWaitForCompletion(s);
}
