...
def attn_fwd(
    ...
    if ENABLE_DROPOUT:
        batch_philox_offset = philox_offset_base + off_z * seqlen_q * seqlen_k
    else:
...
def run_benchmark(configs: list[tuple[int, int, int, int, int, str]]):
    def bench_flash_attention(
        ...
        dtype_torch = arg_to_torch_dtype[dtype]

        q, k, v, input_metadata = input_helper(BATCH, NH, NH, N_CTX_Q, N_CTX_K, D_HEAD, dtype_torch)
        ...
        flops_per_matmul = 2.0 * BATCH * NH * N_CTX_Q * N_CTX_K * D_HEAD
        total_flops = 2 * flops_per_matmul
        total_bytes = BATCH * NH * 2 * (2 * N_CTX_K * D_HEAD + 2 * N_CTX_Q * D_HEAD + N_CTX_Q * N_CTX_K)
        
        times = torch.tensor([s.elapsed_time(e) for s, e in zip(start_event, end_event)], dtype=torch.float)
        mean_microseconds = times.mean().item() * 1000

        return {
            "mean_microseconds": mean_microseconds,
            "arithmetic_intensity": total_flops / total_bytes,
            "tflops": total_flops / mean_microseconds * 1e-6,
        }
